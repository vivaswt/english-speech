...
class YouTubeBatch extends ChangeNotifier {
  ...
  Future<void> start() async {
    ...
    for (int i = 0; i < _batchItems.length; i++) {
      ...
      try {
        ...
        final content = await getYouTubeSummurizedContent(item.url);

        if (_batchStatus != BatchStatus.processing) break;
        await registForTTS(
          TTSContent(title: item.title, url: item.url, content: content),
        );
